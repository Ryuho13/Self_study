...
// 답 :
// 스택에 인덱스를 쌓으면서, 현재 가격이 스택 top의 가격보다 낮으면(= 가격 하락)
// pop하여 "하락 직전까지의 초 수"를 기록. 끝까지 남은 인덱스는 마지막까지 버텼으므로 (n-1-i).
...
// 답 :
// 시간마다 필요 서버 수(players[i]/m)를 구하고,
// 슬라이딩 윈도우(크기 k)로 현재 운영 중인 서버 수를 추적.
// 부족분만큼 그 시간에 증설하고 누적 합산.

class Solution {
    public int solution(int[] players, int m, int k) {
        int[] deployed = new int[24];
        int total = 0;
